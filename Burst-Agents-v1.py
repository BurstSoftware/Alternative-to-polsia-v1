...
# ====================== ALL AGENTS (Provider removed) ======================
DEFAULT_AGENTS = {
    "🔍 Researcher": {"role": "Expert Researcher", "goal": "Gather accurate, balanced, and well-sourced information", "backstory": "You are a meticulous researcher who values evidence and multiple perspectives.", "system_prompt": "You are an expert researcher. Provide detailed, accurate, objective information. Use evidence-based reasoning and present balanced views when appropriate."},
    "✍️ Creative Writer": {"role": "Creative Writer", "goal": "Create engaging, original, and high-quality written content", "backstory": "You have a vivid imagination and mastery of storytelling and persuasive writing.", "system_prompt": "You are a highly creative writer. Use vivid language, strong narratives, and the appropriate tone for the request. Be original and engaging."},
    "💻 Code Expert": {"role": "Senior Software Engineer", "goal": "Write, debug, review, and explain high-quality code", "backstory": "You are a seasoned developer who follows best practices and writes clean, efficient code.", "system_prompt": "You are an expert software engineer. Write clean, well-commented code. Always explain your reasoning and suggest improvements."},
...
def get_client(provider: str):
    if provider == "nvidia":
        ...
        if not st.session_state.mistral_api_key:
            ...
            return None
        try:
            return OpenAI(base_url="https://api.mistral.ai/v1", api_key=st.session_state.mistral_api_key)
        except Exception as e:
...
with st.sidebar:
    ...
    st.session_state.api_key = st.text_input("NVIDIA NIM API Key", type="password", value=st.session_state.api_key)
    st.session_state.mistral_api_key = st.text_input("Mistral API Key", type="password", value=st.session_state.mistral_api_key)

    st.slider("Temperature", 0.0, 1.0, st.session_state.temperature, 0.05, key="temperature")
    ...
    st.caption("Powered by NVIDIA NIM + Mistral • Flexible Agent Studio")

# ====================== MAIN APP ======================
st.markdown('<h1 class="main-header">🤖 NVIDIA NIM Agent Studio</h1>', unsafe_allow_html=True)

agents = DEFAULT_AGENTS
selected_agent_name = st.selectbox("🎭 Select Agent", list(agents.keys()), key="agent_select")

base_agent = agents[selected_agent_name]

# Provider + Model Selection
col1, col2 = st.columns([1, 1])
with col1:
    provider = st.radio("Backend", ["nvidia", "mistral"], horizontal=True, key="provider_radio")
...
with col2:
    if provider == "nvidia":
        ...
        model_options = ["mistral-large-latest", "mistral-small-latest", "open-mistral-nemo"]
        model = st.selectbox("Model", model_options, index=0)
...
st.info(f"**Active Agent:** {selected_agent_name} | **Provider:** {provider.upper()} | **Model:** {model}")
...
if prompt := st.chat_input("Type your message here..."):
    ...
    with st.chat_message("assistant", avatar="🤖"):
        ...
        response = generate_response(prompt, current_agent)
